...
var getRecursiveNodeData = function(filterFunction) {
  var result = [];

  if(filterFunction) {
    var singleNodeData = this.getSingleNodeData();

    if(singleNodeData && singleNodeData.length > 0) {
      for(var i=0;i<singleNodeData.length;i++) {
        var dataItem = singleNodeData[i];
